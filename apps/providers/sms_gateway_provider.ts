...
function registerApi(app: Express, type: "sms" | "email", index: number) {
  ...
  app.get("/health", (_req: Request, res: Response) => {
    res.status(200).json({ status: "healthy", provider: `provider${index}`, type });
  });
...
// Determine which provider instance this container should run based on service identification
// If SERVICE_PROVIDER_INDEX is set, run only that provider; otherwise run all for backward compatibility
const serviceProviderIndex = process.env.SERVICE_PROVIDER_INDEX ? parseInt(process.env.SERVICE_PROVIDER_INDEX) : null;

if (serviceProviderIndex) {
  // Run only the specified provider instance
  const index = serviceProviderIndex;
  ...
  smsApp.listen(Number(smsPort), '0.0.0.0', () => {
    console.log(`SMS Provider ${index} running on port ${smsPort}`);
  });
  emailApp.listen(Number(emailPort), '0.0.0.0', () => {
    console.log(`Email Provider ${index} running on port ${emailPort}`);
  });
} else {
  // Run all provider instances (for backward compatibility)
