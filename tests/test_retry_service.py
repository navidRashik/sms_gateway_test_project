...
class TestRetryServiceInitialization:
    ...
    def test_init(self, retry_service, mock_redis, mock_db_session, mock_health_tracker):
        ...
        # Check provider URLs are set correctly
        assert retry_service.providers["provider1"] == "http://provider1.com"
        assert retry_service.providers["provider2"] == "http://provider2.com"
        assert retry_service.providers["provider3"] == "http://provider3.com"

    def test_init_custom_parameters(self, mock_redis, mock_db_session, mock_health_tracker):
...
class TestExponentialBackoff:
    ...
    def test_calculate_backoff_delay_first_retry(self, retry_service):
        """Test backoff delay for first retry (attempt 1)."""
        delay = retry_service.calculate_backoff_delay(1)
        expected_delay = 1.0 * (2 ** 1)  # 2.0 seconds

        assert delay == expected_delay

    def test_calculate_backoff_delay_second_retry(self, retry_service):
        """Test backoff delay for second retry (attempt 2)."""
        delay = retry_service.calculate_backoff_delay(2)
        expected_delay = 1.0 * (2 ** 2)  # 4.0 seconds

        assert delay == expected_delay

    def test_calculate_backoff_delay_max_delay(self, retry_service):
        """Test that delay is capped at max_delay."""
        # High attempt number that would exceed max_delay
        delay = retry_service.calculate_backoff_delay(10)
    ...
    def test_calculate_backoff_delay_no_jitter(self, mock_redis, mock_db_session, mock_health_tracker):
...
class TestProviderManagement:
    ...
    @pytest.mark.asyncio
    async def test_get_failed_providers_database_error(self, retry_service, mock_db_session):
    ...
    @pytest.mark.asyncio
    async def test_get_healthy_providers_all_healthy(self, retry_service, mock_health_tracker):
    ...
    @pytest.mark.asyncio
    async def test_get_healthy_providers_some_unhealthy(self, retry_service, mock_health_tracker):
    ...
    @pytest.mark.asyncio
    async def test_get_healthy_providers_with_exclusions(self, retry_service, mock_health_tracker):
    ...
    @pytest.mark.asyncio
    async def test_get_healthy_providers_all_unhealthy(self, retry_service, mock_health_tracker):
    ...
    @pytest.mark.asyncio
    async def test_select_provider_round_robin_all_healthy(self, retry_service, mock_health_tracker):
    ...
    @pytest.mark.asyncio
    async def test_select_provider_round_robin_with_exclusions(self, retry_service, mock_health_tracker):
    ...
    @pytest.mark.asyncio
    async def test_select_provider_round_robin_no_healthy_providers(self, retry_service, mock_health_tracker):
...
class TestRetryAttemptRecording:
    ...
    @pytest.mark.asyncio
    async def test_record_retry_attempt_database_error(self, retry_service, mock_db_session):
...
class TestRequestStatusUpdates:
    ...
    @pytest.mark.asyncio
    async def test_update_request_retry_status_success(self, retry_service, mock_db_session):
    ...
    @pytest.mark.asyncio
    async def test_update_request_retry_status_permanently_failed(self, retry_service, mock_db_session):
    ...
    @pytest.mark.asyncio
    async def test_update_request_retry_status_request_not_found(self, retry_service, mock_db_session):
    ...
    @pytest.mark.asyncio
    async def test_mark_request_permanently_failed(self, retry_service, mock_db_session):
...
class TestRetryDecisionLogic:
    ...
    @pytest.mark.asyncio
    async def test_should_retry_max_retries_exceeded(self, retry_service):
    ...
    @pytest.mark.asyncio
    async def test_should_retry_no_healthy_providers(self, retry_service, mock_health_tracker):
    ...
    @pytest.mark.asyncio
    async def test_should_retry_successful_selection(self, retry_service, mock_health_tracker):
        ...
        assert delay == 2.0  # 1.0 * (2 ** 1) with jitter

    @pytest.mark.asyncio
    async def test_should_retry_multiple_failed_providers(self, retry_service, mock_health_tracker, mock_db_session):
        ...
        # Only provider3 is healthy
        mock_health_tracker.is_provider_healthy.side_effect = [False, False, True]
        ...
        assert should_retry is True
        assert next_provider == "provider3"  # Should select the only healthy provider
        assert delay == 4.0  # 1.0 * (2 ** 2) with jitter
...
class TestRetryExecution:
    ...
    async def test_execute_retry_with_backoff_success(self, retry_service, mock_db_session):
        ...
        with patch('src.retry_service.send_sms_to_provider') as mock_send_task:
            ...
            assert result["message_id"] == "retry_123_1"
            assert result["provider_id"] == "provider2"

            # Verify retry was recorded in database
            mock_db_session.add.assert_called_once()
            mock_db_session.commit.assert_called_once()

            # Verify task was called with correct parameters
            mock_send_task.kicker.assert_called_once()
    ...
    async def test_execute_retry_with_backoff_exception(self, retry_service, mock_db_session):
        ...
        with patch('src.retry_service.send_sms_to_provider') as mock_send_task:
            mock_send_task.kicker = AsyncMock(side_effect=Exception("Task execution error"))
            ...
            assert result["success"] is False
            assert "Retry execution error" in result["error"]
...
class TestRetryScenarios:
    ...
    async def test_complete_retry_cycle_success(self, retry_service, mock_health_tracker, mock_db_session):
        ...
        with patch('src.retry_service.send_sms_to_provider') as mock_send_task:
            mock_send_task.kicker = AsyncMock(return_value={"success": True})
    ...
    async def test_health_tracker_error_handling(self, retry_service, mock_health_tracker):
        ...
        # Health tracker throws an error
        mock_health_tracker.is_provider_healthy.side_effect = Exception("Health tracker error")
        ...
        # Should default to no retry on health tracker error
        assert should_retry is False
        assert next_provider is None